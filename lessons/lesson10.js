...
LESSONS.push({
  ...
  goal: { text: '让实验室跑过 60 帧，然后答对右侧全部 3 道毕业题。', hint: '实验室默认就在跑；三题对应第 6、8、9 关' },
  notes: [
    {
    ...
    },
    {
      ...
      html: '双射（第 6 关）、ping-pong（第 8 关）、帧数演化（第 9 关）——正好是整门课的骨架。',
    },
    {
      near: '.fig', title: '去玩真的',
      html: '主程序在 8944 端口（双击项目里的 start.bat）。按 <b>R</b> 重新播种，观察真实紫金花的完整演化。',
    },
  ],
  build(demoArea, api) {
    const W = 168, H = 108;
    ...
    function detailSlotBadge(apiRef) {
      api.detailSlot.appendChild(
        api.el('div', 'badge-line',
          ...
          '<div class="ds">去玩真正的紫金花：<a href="https://www.shadertoy.com/view/73c3R7" target="_blank">Shadertoy 原作</a> · 本地主程序 start.bat（端口 8944）</div>')
      );
    }
  },
  theory: {
    story:
      ...
      '<p>想继续深入：读 <b>shaders/bufferA.frag</b>（原文注释很棒）、<b>analysis/cpu_sim.js</b>（float64 仲裁基准）、以及 README 里的「已知行为（非 bug）」一节。</p>',
  },
